...
model_3_i <- function(dependent,cluster=T) {
  # removes entries with na values
   updated_data<-individual_dat%>%filter(!is.na(dependent) & !is.na(individual_dat$idcluster))
   # control variables
   x1<- updated_data$S_HHpeople
   x2<-updated_data$S_rooms
   x3<-updated_data$S_age
   x4<-updated_data$S_gender
   x5<-updated_data$S_childma
   x6<-updated_data$S_childmaage
   x7<-updated_data$S_childmaeduc 
   x8<-updated_data$S_childpa
   x9<-updated_data$S_childpaage 
   x10<-updated_data$S_childpaeduc
   x11<-updated_data$S_waterland
   x12<-updated_data$S_waterhouse 
   x13<-updated_data$S_electricity
   x14<-updated_data$S_hasanimals
   x15<-updated_data$S_animalsinside
   x16<-updated_data$S_garbage
   x17<-updated_data$S_washhands
   x18<-updated_data$S_cashtransfers
   x19<-updated_data$S_milkprogram
   x20<-updated_data$S_foodprogram
   x21<-updated_data$S_seguropopular
   x22<- updated_data$dpisofirme
   updated_dependent<- dependent[!is.na(dependent)& !is.na(individual_dat$idcluster)]
  if(cluster==T)
  {
    return(tidy(lm_robust(updated_dependent ~ x22 + x1 + x2 + x3  + x4+ x5+ x6+ x7+ x8+ x9+ x10+ x11+ x12+ x13+ x14+ x15+ x16+ x17+x18+x19+x20+x21+x22,updated_data,clusters=idcluster)))
  }else{
...
```{r r-sq-plots}
# join with df that contain coefficients
...
# plot coefficients versus r squared values
ggplot(data = T4_2) + geom_point(mapping = aes(x = Model_1, y = r_sq_m1)) + xlab("Model 1 Coefficients") + ylab("Model 1 R Squared") + ggtitle("Table 4")
ggplot(data = T4_2) + geom_point(mapping = aes(x = Model_2, y = r_sq_m2)) + xlab("Model 2 Coefficients") + ylab("Model 2 R Squared") + ggtitle("Table 4")
ggplot(data = T4_2) + geom_point(mapping = aes(x = Model_3, y = r_sq_m3)) + xlab("Model 3 Coefficients") + ylab("Model 3 R Squared") + ggtitle("Table 4")

ggplot(data = T5_2) + geom_point(mapping = aes(x = Model_1, y = r_sq_m1)) + xlab("Model 1 Coefficients") + ylab("Model 1 R Squared") + ggtitle("Table 5")
ggplot(data = T5_2) + geom_point(mapping = aes(x = Model_2, y = r_sq_m2)) + xlab("Model 2 Coefficients") + ylab("Model 2 R Squared") + ggtitle("Table 5")
ggplot(data = T5_2) + geom_point(mapping = aes(x = Model_3, y = r_sq_m3)) + xlab("Model 3 Coefficients") + ylab("Model 3 R Squared") + ggtitle("Table 5")

ggplot(data = T6_2) + geom_point(mapping = aes(x = Model_1, y = r_sq_m1)) + xlab("Model 1 Coefficients") + ylab("Model 1 R Squared") + ggtitle("Table 6")
ggplot(data = T6_2) + geom_point(mapping = aes(x = Model_2, y = r_sq_m2)) + xlab("Model 2 Coefficients") + ylab("Model 2 R Squared") + ggtitle("Table 6")
ggplot(data = T6_2) + geom_point(mapping = aes(x = Model_3, y = r_sq_m3)) + xlab("Model 3 Coefficients") + ylab("Model 3 R Squared") + ggtitle("Table 6")
```
